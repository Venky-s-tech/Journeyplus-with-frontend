    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-card border border-border rounded-lg shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">ID: #{claim.id} | Submitted: {formatDate(claim.submittedDate)}</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {isEmployee && claim.status === "DRAFT" && (
            <>
              <Button onClick={handleSubmitClaim} disabled={claim.expenseLines?.length === 0}>
                Submit entire Claim
              </Button>
            </>
          )}
          ...
          <div className="p-4 bg-card border border-border rounded-lg shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-primary">{formatCurrency(claim.totalAmount, claim.originalCurrency)}</span>
              </div>
              <div className="flex justify-between border-t pt-2 mt-2">
                <span className="text-muted-foreground">Reimbursable Value:</span>
                <span className="font-bold text-lg text-primary">{formatCurrency(claim.totalAmount, claim.originalCurrency)}</span>
              </div>
            </div>
          </div>
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="p-4 bg-card border border-border rounded-lg shadow-sm space-y-4">
                ...
                <Dialog open={isLineDialogOpen} onOpenChange={setIsLineDialogOpen}>
                  ...
                  <DialogContent className="max-w-sm">
                    ...
                    <form onSubmit={handleAddLine} className="space-y-3 text-xs">
                      ...
                      <div className="space-y-1">
                        <Label>Receipt File Upload</Label>
                        <div className="flex gap-2">
                          <Input
                            type="file"
                        ...
                        {isUploading && <p className="text-[10px] text-muted-foreground">Uploading...</p>}
                        {receiptPath && (
                          <p className="text-[10px] text-green-600 truncate">Uploaded path: {receiptPath}</p>
                        )}
                      </div>
            ...
            <div className="divide-y divide-border">
              {!claim.expenseLines || claim.expenseLines.length === 0 ? (
                  ...
                  <div key={line.id} className="py-3 flex items-center justify-between text-xs gap-3">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        ...
                        <StatusBadge status={line.complianceStatus || "PENDING"} />
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isEmployee && claim.status === "DRAFT" && line.complianceStatus !== "INCLUDED" && (
